...
# Kept independent from the GPL Blender Module by the versioned file boundary.
...
def parse_json_bytes(data: bytes, *, maximum: int = MAX_DOCUMENT_BYTES) -> Any:
    ...
    try:
        value = json.loads(data.decode("utf-8", errors="strict"), object_pairs_hook=_reject_duplicates, parse_constant=_reject_constant)
